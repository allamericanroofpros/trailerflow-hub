      ...
      {view === "register" ? (
        <div className="flex flex-1 overflow-hidden">
          ...
          <div className="flex-1 flex flex-col overflow-hidden">
            ...
            {/* Favorites Row */}
            {menuItems && menuItems.length > 0 && activeCategory === "all" && (
              <div className="flex items-center gap-2 px-4 py-2.5 border-b border-border overflow-x-auto scrollbar-hide shrink-0">
                <Star className="h-4 w-4 text-warning shrink-0" />
                {menuItems.slice(0, 5).map((item) => (
                  <button
                    key={item.id}
                    onClick={() => handleItemTap(item)}
                    className="shrink-0 flex items-center gap-1.5 rounded-xl bg-warning/10 border border-warning/30 px-3 py-2 text-xs font-bold text-warning hover:bg-warning/20 active:scale-95 transition-all touch-manipulation"
                  >
                    <Plus className="h-3.5 w-3.5" /> {item.name}
                  </button>
                ))}
              </div>
            )}

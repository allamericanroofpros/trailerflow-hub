        ...
        {activeSection === "billing" && (
          <div className="rounded-xl border border-border bg-card p-6 shadow-card">
            <div className="flex items-center justify-between mb-5">
              ...
              {subscribed && (
                <Button variant="outline" size="sm" onClick={async () => { try { await openPortal(); } catch { toast.error("Could not open billing portal"); } }}>
                  <ExternalLink className="h-3.5 w-3.5 mr-1.5" /> Manage Subscription
                </Button>
              )}
            </div>
                    ...
                    return (
                      <div
                        ...
                        className={`relative rounded-xl border p-5 transition-all ${
                          isCurrent ? "border-primary ring-2 ring-primary/20 bg-primary/5" : "border-border bg-background hover:border-primary/40"
                        } ${key === "pro" && !isCurrent ? "ring-1 ring-primary/10" : ""}`}
                      >
                        {key === "pro" && !isCurrent && (
                          <span className="absolute -top-2.5 left-4 rounded-full bg-primary px-2.5 py-0.5 text-[10px] font-bold text-primary-foreground uppercase tracking-wider">
                            Most Popular
                          </span>
                        )}
                        {isCurrent && (
                        ...
                        {isCurrent ? (
                          ...
                          <Button
                            ...
                            onClick={async () => {
                              try {
                                await startCheckout(t.price_id);
                              } catch (e: any) {

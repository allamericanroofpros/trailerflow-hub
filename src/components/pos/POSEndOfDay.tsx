...
const POSEndOfDay = forwardRef<HTMLDivElement, { onClose: () => void }>(
  function POSEndOfDay({ onClose }, ref) {
    ...
    const cashVariance = useMemo(() => {
      if (!cashDrawerStart || !cashDrawerEnd || !todayStats) return null;
      const expected = Number(cashDrawerStart) + todayStats.cashSales;
      const actual = Number(cashDrawerEnd);
      return { expected, actual, variance: actual - expected };
    }, [cashDrawerStart, cashDrawerEnd, todayStats]);
    ...
    const handleCompleteEOD = async () => {
      setSaving(true);
      try {
        // Update inventory counts if provided
            ...
            {step === "cash" && todayStats && (
              <div className="space-y-6">
                <div className="text-center">
                  ...
                  <p className="text-sm text-muted-foreground mt-1">Count your cash drawer to reconcile</p>
                </div>

                <div className="space-y-4">
                  <div>
                    <label className="text-sm font-bold text-muted-foreground">Opening Drawer ($)</label>
                    <Input
                      type="number"
                      step="0.01"
                      value={cashDrawerStart}
                      onChange={(e) => setCashDrawerStart(e.target.value)}
                      placeholder="How much cash started the day?"
                      className="mt-1 h-12 rounded-xl border-2 text-lg font-bold text-center"
                    />
                  </div>
                  <div>
                    <label className="text-sm font-bold text-muted-foreground">Closing Drawer ($)</label>
                    <Input
                      type="number"
                      step="0.01"
                      value={cashDrawerEnd}
                      onChange={(e) => setCashDrawerEnd(e.target.value)}
                      placeholder="Count all cash now in drawer"
                      className="mt-1 h-12 rounded-xl border-2 text-lg font-bold text-center"
                    />
                  </div>
                  ...
                  {cashVariance && (
                    <div className={`rounded-2xl border-2 p-4 text-center ${
                      ...
                      <p className="text-xs font-bold text-muted-foreground">Expected: ${cashVariance.expected.toFixed(2)}</p>
            ...
            {step === "inventory" && (
              <div className="space-y-6">
                <div className="text-center">
                  ...
                  <p className="text-sm text-muted-foreground mt-1">Count key items — skip any you don't need to count</p>
                </div>
                ...
                <div className="space-y-2">
                  {inventoryItems?.map(item => {
                    ...
                    const variance = counted !== undefined && counted !== ""
                      ? Number(counted) - Number(item.current_stock)
                      : null;
                    return (
              ...
              <div className="space-y-6">
                ...
                <div>
                  ...
                  <Textarea
                    ...
                    rows={3}
                    placeholder="Anything to note for tomorrow? Equipment issues, supply needs..."
                    className="mt-1"
                  />
                </div>

                <Button
                  className="w-full h-12 font-black rounded-xl gap-2"
            ...
            {step === "complete" && (
              <div className="text-center space-y-6 py-8">
                ...
                <p className="text-sm text-muted-foreground">
                  Inventory has been updated, cash reconciled, and trailer shutdown complete. Great day!
                </p>
                ...
                <Button className="w-full h-12 font-black rounded-xl" onClick={onClose}>
                  Close
                </Button>
              </div>
            )}
